...
def format_ble_table(data, max_lines=None, max_cols=None):
    ...
    if max_cols is None:
        ...
        # Curses mode: tabulated layout with line splitting
        COL_WIDTHS = {
            'name': 36,
            'props': 10,
            'value': 40,
        ...
        for service in data:
            ...
            for char_data in char_list_data:
                ...
                name_wrapped = []
                current_name = char_data['name']
                while current_name:
                    name_wrapped.append(current_name[:COL_WIDTHS['name']])
                    current_name = current_name[COL_WIDTHS['name']:]
                
                props_wrapped = []
                current_props = char_data['props']
                while current_props:
                    props_wrapped.append(current_props[:COL_WIDTHS['props']])
                    current_props = current_props[COL_WIDTHS['props']:]

                hex_lines = []
                if char_data['value_chunks']:
                    for i in range(0, len(char_data['value_chunks']), 8):
                        line = ' '.join(char_data['value_chunks'][i:i+8])
                        current_line_segment = line
                        while current_line_segment:
                            hex_lines.append(current_line_segment[:COL_WIDTHS['value']])
                            current_line_segment = current_line_segment[COL_WIDTHS['value']:]
                else:
                    current_line_segment = char_data['value']
                    while current_line_segment:
                        hex_lines.append(current_line_segment[:COL_WIDTHS['value']])
                        current_line_segment = current_line_segment[COL_WIDTHS['value']:]
                ...
                if char_data['parsed']:
                    for desc, val in char_data['parsed']:
                        line_to_add = f"{desc}: {val}"
                        current_line_segment = line_to_add
                        while current_line_segment:
                            parsed_lines.append(current_line_segment[:COL_WIDTHS['parsed']])
                            current_line_segment = current_line_segment[COL_WIDTHS['parsed']:]
                else:
                    parsed_lines.append("")
